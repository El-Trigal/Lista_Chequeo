...
import { downloadSopladoRecordsExcel, getCurrentWeekCode } from "./lib/excelExport";
...
const ASSIGNED_BEDS = 32;
const RENDIMIENTO_SCORE = 10;
const REQUIREMENTS = [
  {
    id: "tapaoidos",
    label: "El soplador cuenta con tapaoídos",
    criterion: "Cuenta con tapaoídos en copa para realizar la labor.",
    weight: 5
  },
  {
    id: "guantes",
    label: "El soplador cuenta con guantes Kimberly",
    criterion: "Cuenta con guantes Kimberly en buen estado.",
    weight: 5
  },
  {
    id: "dispositivos",
    label: "Uso de dispositivos electrónicos",
    criterion: "Usa adecuadamente los dispositivos electrónicos durante la labor.",
    weight: 20
  },
  {
    id: "almacenamiento",
    label: "Se almacena la máquina de forma adecuada",
    criterion: "La máquina se almacena en la ubicación definida y con el plástico en buen estado.",
    weight: 5
  },
  {
    id: "pantalla",
    label: "La pantalla se encuentra en buen estado",
    criterion: "La pantalla está limpia y sin agujeros.",
    weight: 10
  }
];
const QUALITY_ITEMS = [
  {
    id: "tiempo_cama",
    label: "Las camas aseguradas se hacen en el tiempo correspondiente",
    criterion: "Las camas aseguradas se realizan en el tiempo correspondiente: 3 min/cama.",
    weight: 10
  },
  {
    id: "horarios",
    label: "El soplador da inicio o descansa en los horarios establecidos",
    criterion: "Inicia y toma descansos en los horarios establecidos: 10 minutos/hora.",
    weight: 5
  },
  {
    id: "ubicacion",
    label: "El soplador está ubicado en la nave correspondiente de la hora",
    criterion: "Al momento del aseguramiento está ubicado en la nave correspondiente de la hora.",
    weight: 5
  },
  {
    id: "desglose",
    label: "Desglose de la labor",
    criterion: "El grupo cumple con el desglose de la labor.",
    weight: 20
  }
];
...
function createInitialForm() {
  return {
    ...
    assurerName: "",
    monitoredBeds: "",
    requirements: createAnswerMap(REQUIREMENTS),
    quality: createAnswerMap(QUALITY_ITEMS),
    observations: ""
  };
}
...
function sanitizeMonitoredBedsInput(value) {
  ...
  return Number.isFinite(numericValue) && numericValue > ASSIGNED_BEDS
    ? String(ASSIGNED_BEDS)
    : sanitizedValue;
}
...
function isRendimientoComplete(form) {
  return Boolean(form.blowerName.trim() && form.assurerName.trim() && String(form.monitoredBeds).trim());
}

function calculateSopladoScore(form) {
  ...
  if (String(form.monitoredBeds).trim()) {
    const row = {
      sectionTitle: "Camas monitoreadas",
      itemLabel: "Número de camas sopladas por hora",
      criterion: `${formatNumber(Number(form.monitoredBeds) || 0)} de ${ASSIGNED_BEDS} camas asignadas por hora.`,
...
function SectionHeader({ number, title, expanded, onToggle, rightSlot }) {
  return (
    <div className="section-heading">
      <div><span className="section-index">{number}</span><h2>{title}</h2></div>
  ...
  return (
    <section className={compact ? "record-summary" : "summary-panel"}>
      <div className={compact ? "record-summary-title" : "summary-top"}>
        {compact ? (
          <><strong>Resumen del registro</strong><span>{formatNumber(result.totalScore)} / {formatNumber(TOTAL_SCORE)} - {formatNumber(result.percent)}%</span></>
        ) : (
          <><div><span className="section-index">Resumen</span><h2>Resultado del chequeo</h2></div><div className="score-card"><span>Calificación</span><strong>{formatNumber(result.totalScore)} / {formatNumber(TOTAL_SCORE)}</strong><small>{formatNumber(result.percent)}% cumplimiento</small></div></>
        )}
      </div>
      ...
      {canSave ? <div className="summary-actions"><button type="button" className="primary-action" onClick={onSave}>Guardar registro</button></div> : null}
    </section>
  );
}
...
function RendimientoSection({ form, expanded, onToggle, onChange, readOnly }) {
  ...
  return (
    <section className={complete ? "section-band completed-section" : "section-band"}>
      <SectionHeader number="1" title="Camas monitoreadas" expanded={expanded} onToggle={onToggle} rightSlot={<div className="section-score">{formatNumber(score)} / {formatNumber(RENDIMIENTO_SCORE)}</div>} />
      {expanded ? (
...
function ItemsSection({ number, title, items, answers, score, total, expanded, onToggle, onAnswerChange, readOnly }) {
  ...
  return (
    <section className={complete ? "section-band completed-section" : "section-band"}>
      <SectionHeader number={number} title={title} expanded={expanded} onToggle={onToggle} rightSlot={<div className="section-score">{formatNumber(score)} / {formatNumber(total)}</div>} />
      {expanded ? (
        <div className="collapsible-content">
          <div className="item-table without-value monitoring-control-table">
            <div className="item-table-head"><span>Item</span><span>Criterio</span><span>Peso</span><span>Cumple</span></div>
            {items.map((item) => (
              <div className="item-row" key={item.id}>
                <div className="item-title">{item.label}</div>
                <div>{item.criterion}</div>
                <div>{item.weight}</div>
                <div><StatusToggle value={answers[item.id]} disabled={readOnly} onChange={(value) => onAnswerChange(item.id, value)} /></div>
              </div>
            ))}
          </div>
        </div>
...
function SopladoStartScreen({ saveState, permissions, onCreate }) {
  return (
    <section className="checklist-start">
      <div><p className="eyebrow">Chequeo</p><h2>Aseguramiento de Soplado</h2><p>Inicia un nuevo registro para desplegar las secciones del chequeo.</p></div>
      {permissions.canCreateChecklists ? <button type="button" className="primary-action create-checklist-button" onClick={onCreate}>Crear Chequeo</button> : <p className="permission-note">Tu usuario puede ver registros, pero no crear chequeos.</p>}
...
function SopladoRecords({ records, recordsSource, isLoading, permissions, onEditRecord }) {
  ...
  function getFilterValues(record) {
    return { week: record.weekCode, date: record.savedDate, collaborator: record.form?.blowerName, assurer: record.form?.assurerName };
  }
        ...
        {isLoading ? <RecordsLoadingState /> : filteredRecords.length ? filteredRecords.map((record) => (
          <div className="rb-record-wrapper" key={record.id}>
            <div role="button" tabIndex={0} className={expandedRecordId === record.id ? "rb-records-row expanded" : "rb-records-row"} onClick={() => setExpandedRecordId(expandedRecordId === record.id ? null : record.id)}>
              ...
              <span>{record.syncStatus === "pending" ? <em className="sync-status-pill">Pendiente</em> : null}<button type="button" className="edit-record-button" onClick={(event) => { event.stopPropagation(); onEditRecord(record); }}>{permissions.canEditRecords ? "Editar" : "Ver"}</button></span>
            </div>
...
export default function SopladoChecklistApp({ currentUser, permissions, onHome, onLogout }) {
  ...
  async function handleSaveRecord() {
    ...
    setSaveState({ type: "success-message", message: isPending ? (editingRecord ? "Registro actualizado localmente. Se sincronizará con Supabase cuando haya conexión." : "Registro guardado localmente. Se sincronizará con Supabase cuando haya conexión.") : (editingRecord ? "Registro actualizado." : "Registro guardado.") });
  ...
  return (
    <main className="app-shell">
      ...
      {view === CHECKLIST_VIEW ? (isChecklistActive ? (
        <>
          <section className="progress-strip">
            <div><span>Secciones completas</span><strong>{answeredCount} / 3</strong></div>
            <div><span>Calificación</span><strong>{formatNumber(result.totalScore)} / {TOTAL_SCORE}</strong></div>
          ...
          <RendimientoSection form={form} expanded={expandedSections.rendimiento} onToggle={() => toggleSection("rendimiento")} onChange={updateForm} readOnly={!permissions.canEditRecords} />
          <ItemsSection number="2" title="Requerimientos" items={REQUIREMENTS} answers={form.requirements} score={result.requirementsScore} total={REQUIREMENTS_TOTAL} expanded={expandedSections.requerimientos} onToggle={() => toggleSection("requerimientos")} onAnswerChange={updateRequirement} readOnly={!permissions.canEditRecords} />
          <ItemsSection number="3" title="Calidad" items={QUALITY_ITEMS} answers={form.quality} score={result.qualityScore} total={QUALITY_TOTAL} expanded={expandedSections.calidad} onToggle={() => toggleSection("calidad")} onAnswerChange={updateQuality} readOnly={!permissions.canEditRecords} />
          <section className="section-band">
            <div className="section-heading"><div><span className="section-index">Observaciones</span><h2>Observaciones</h2></div></div>
            <textarea className="observations-box" value={form.observations} readOnly={!permissions.canEditRecords} onChange={(event) => updateForm({ observations: event.target.value })} />
          </section>
