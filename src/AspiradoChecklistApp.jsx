...
import { deleteAspiradoRecord, loadAspiradoRecords, saveAspiradoRecord, updateAspiradoRecord } from "./lib/aspiradoRecords";
...
function AspiradoRecords({ sede, records, recordsSource, isLoading, permissions, onEditRecord }) {
  ...
  function handleDownloadExcel() { if (!filteredRecords.length) { window.alert("No hay registros para descargar con los filtros actuales."); return; } downloadAspiradoRecordsExcel(sede, filteredRecords); }
  return <section className="records-section"><div className="records-heading"><div><span className="section-index">Registros</span><h2>Chequeos guardados</h2></div><div className="records-actions"><RecordFilters options={filterOptions} draftFilters={draftFilters} appliedFilters={appliedFilters} onToggle={toggleFilter} onApply={applyFilters} onClear={clearFilters} collaboratorLabel="Aspirador/a" />{permissions.canDownloadExcel ? <button type="button" className="secondary-action" onClick={handleDownloadExcel}>Descargar Excel</button> : null}<span className="source-pill">{recordsSource}</span></div></div><div className="rb-records-table"><div className="rb-records-head"><span>Aspirador/a</span><span>Asegurador/a</span><span>Fecha</span><span>Semana</span><span>Calificación</span><span>%</span><span>Acción</span></div>{isLoading ? <RecordsLoadingState /> : filteredRecords.length ? filteredRecords.map((record) => <div className="rb-record-wrapper" key={record.id}><div role="button" tabIndex={0} className={expandedRecordId === record.id ? "rb-records-row expanded" : "rb-records-row"} onClick={() => setExpandedRecordId(expandedRecordId === record.id ? null : record.id)}><span>{record.form?.aspiratorName || "Sin aspirador/a"}</span><span>{record.form?.assurerName || "Sin asegurador/a"}</span><span>{record.savedDate || "-"}</span><span>{record.weekCode || "-"}</span><span>{formatNumber(record.score)} / {TOTAL_SCORE}</span><span>{formatNumber(record.percent)}%</span><span>{record.syncStatus === "pending" ? <em className="sync-status-pill">Pendiente</em> : null}<button type="button" className="edit-record-button" onClick={(event) => { event.stopPropagation(); onEditRecord(record); }}>{permissions.canEditRecords ? "Editar" : "Ver"}</button></span></div>{expandedRecordId === record.id ? <SummaryTable result={{ ...record.summary, totalScore: record.score, percent: record.percent }} observations={record.form?.observations} compact /> : null}</div>) : <p className="records-empty">No hay registros con los filtros actuales.</p>}</div></section>;
}
